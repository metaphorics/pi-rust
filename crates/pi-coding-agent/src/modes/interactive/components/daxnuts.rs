//! Animated "Powered by daxnuts" easter egg.

use pi_tui::component::{Component, RenderStatus};
use pi_tui::line::Line;
use pi_tui::util::visible_width;

use crate::modes::interactive::theme::{ThemeColor, theme};

// 32x32 RGB image of dax, hex encoded (3 bytes per pixel).
const DAX_HEX: &str = "";
const WIDTH: usize = 32;
const HEIGHT: usize = 32;
const MAX_TICKS: usize = 25;
const RESET: &str = "\x1b[0m";

fn channel(pixel: usize, channel: usize) -> u8 {
    let offset = pixel * 6 + channel * 2;
    u8::from_str_radix(&DAX_HEX[offset..offset + 2], 16).expect("embedded Dax RGB data is valid")
}

fn rgb(r: u8, g: u8, b: u8, background: bool) -> String {
    format!("\x1b[{};2;{r};{g};{b}m", if background { 48 } else { 38 })
}

fn build_image() -> Vec<String> {
    let mut lines = Vec::with_capacity(HEIGHT / 2);
    for row in (0..HEIGHT).step_by(2) {
        let mut line = String::new();
        for x in 0..WIDTH {
            let top = row * WIDTH + x;
            let bottom = (row + 1).min(HEIGHT - 1) * WIDTH + x;
            let (tr, tg, tb) = (channel(top, 0), channel(top, 1), channel(top, 2));
            let (br, bg, bb) = (channel(bottom, 0), channel(bottom, 1), channel(bottom, 2));
            line.push_str(&rgb(br, bg, bb, false));
            line.push_str(&rgb(tr, tg, tb, true));
            line.push('▄');
        }
        line.push_str(RESET);
        lines.push(line);
    }
    lines
}

fn centered(text: &str, width: usize) -> String {
    let left = width.saturating_sub(visible_width(text)) / 2;
    format!("{}{text}", " ".repeat(left))
}

pub struct Daxnuts {
    image: Vec<String>,
    tick: usize,
    cached: Vec<Line>,
    cached_width: Option<u16>,
    cached_tick: Option<usize>,
    status: RenderStatus,
}

impl Daxnuts {
    #[must_use]
    pub fn new() -> Self {
        Self {
            image: build_image(),
            tick: 0,
            cached: Vec::new(),
            cached_width: None,
            cached_tick: None,
            status: RenderStatus::Changed,
        }
    }

    /// Advance one 80 ms animation step. No background thread is used.
    pub fn tick(&mut self) -> bool {
        if self.tick < MAX_TICKS {
            self.tick += 1;
            self.cached_width = None;
        }
        self.tick >= MAX_TICKS
    }

    pub fn dispose(&mut self) {}
}

impl Default for Daxnuts {
    fn default() -> Self {
        Self::new()
    }
}

impl Component for Daxnuts {
    fn render(&mut self, width: u16) -> &[Line] {
        if self.cached_width == Some(width) && self.cached_tick == Some(self.tick) {
            self.status = RenderStatus::Unchanged;
            return &self.cached;
        }
        let width_usize = usize::from(width);
        let revealed_rows = self
            .image
            .len()
            .min((self.tick * (self.image.len() + 3)) / MAX_TICKS);
        let mut lines = Vec::with_capacity(self.image.len() + 9);
        lines.push(String::new());
        for (index, image_line) in self.image.iter().enumerate() {
            if index < revealed_rows {
                lines.push(centered(image_line, width_usize));
            } else if index == revealed_rows {
                let scanline = format!(
                    "{}{}{}",
                    rgb(100, 200, 255, false),
                    "▓".repeat(WIDTH),
                    RESET
                );
                lines.push(centered(&scanline, width_usize));
            } else {
                lines.push(centered(&" ".repeat(WIDTH), width_usize));
            }
        }
        lines.push(String::new());
        let text_phase = self.tick.saturating_sub(MAX_TICKS * 3 / 5);
        if text_phase > 0 || self.tick >= MAX_TICKS {
            lines.push(centered(
                &theme().fg(ThemeColor::Accent, "Free Kimi K2.5 via OpenCode Zen"),
                width_usize,
            ));
            lines.push(centered(
                &theme().fg(ThemeColor::Success, "\"Powered by daxnuts\""),
                width_usize,
            ));
            lines.push(centered(
                &theme().fg(ThemeColor::Muted, "— @thdxr"),
                width_usize,
            ));
        } else {
            lines.extend([String::new(), String::new(), String::new()]);
        }
        lines.push(String::new());
        if text_phase > 2 || self.tick >= MAX_TICKS {
            lines.push(centered(
                &theme().fg(ThemeColor::Dim, "Try OpenCode"),
                width_usize,
            ));
            lines.push(centered(
                &theme().fg(
                    ThemeColor::MdLink,
                    "https://mistral.ai/news/mistral-vibe-2-0",
                ),
                width_usize,
            ));
        } else {
            lines.extend([String::new(), String::new()]);
        }
        lines.push(String::new());
        self.cached = lines
            .into_iter()
            .map(|line| Line::from_ansi(&line))
            .collect();
        self.cached_width = Some(width);
        self.cached_tick = Some(self.tick);
        self.status = RenderStatus::Changed;
        &self.cached
    }

    fn invalidate(&mut self) {
        self.cached_width = None;
        self.status = RenderStatus::Changed;
    }

    fn last_render_status(&self) -> RenderStatus {
        self.status
    }
}
