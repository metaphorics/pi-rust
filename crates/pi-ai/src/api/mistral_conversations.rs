...
pub fn build_request_body(model: &Model, context: &Context, options: &StreamOptions) -> Value {
    ...
    let tools = transform_messages::openai_tools(context); if !tools.is_empty() { body["tools"] = Value::Array(tools); }
    if let Some(temperature) = options.temperature { body["temperature"] = json!(temperature); }
    if model.reasoning { body["prompt_mode"] = Value::String("reasoning".into()); }
    body
}
...
where I: IntoIterator<Item=B>, B: AsRef<[u8]> { openai_completions::parse_stream_events(chunks, model) }
...
pub fn stream_with_client(model: Model, context: Context, options: StreamOptions, client: Arc<dyn StreamHttpClient>) -> AssistantMessageEventStream {
    let url = format!("{}/chat/completions", model.base_url.trim_end_matches('/')); let headers = build_headers(&model, &options); let body = build_request_body(&model, &context, &options);
