...
pub fn build_request_body(model: &Model, context: &Context, options: &StreamOptions) -> Value { google_shared::build_request_body(model, context, options) }
...
pub fn build_headers(model: &Model, options: &StreamOptions) -> Vec<(String, String)> {
    ...
    if let Some(token) = &options.api_key { headers.push(("authorization".into(), format!("Bearer {token}"))); }
    headers
}

pub fn build_url(model: &Model, options: &StreamOptions) -> String {
    let metadata = options.metadata.as_ref();
    let project = metadata.and_then(|m| m.get("project")).and_then(Value::as_str)
        .or_else(|| options.env.as_ref().and_then(|e| e.get("GOOGLE_CLOUD_PROJECT").map(String::as_str))).unwrap_or("PROJECT_ID");
    let location = metadata.and_then(|m| m.get("location")).and_then(Value::as_str)
        .or_else(|| options.env.as_ref().and_then(|e| e.get("GOOGLE_CLOUD_LOCATION").map(String::as_str))).unwrap_or("global");
    if model.base_url.contains("{project}") || model.base_url.contains("{location}") {
        return model.base_url.replace("{project}", project).replace("{location}", location).replace("{model}", &model.id);
    }
    let host = if location == "global" { "aiplatform.googleapis.com".to_owned() } else { format!("{location}-aiplatform.googleapis.com") };
    format!("https://{host}/v1/projects/{project}/locations/{location}/publishers/google/models/{}:streamGenerateContent?alt=sse", model.id)
}

pub fn parse_stream_events<I, B>(chunks: I, model: &Model) -> common::ApiResult<Vec<AssistantMessageEvent>>
where I: IntoIterator<Item=B>, B: AsRef<[u8]> { google_shared::parse_google_stream(chunks, model) }
...
pub fn stream_with_client(model: Model, context: Context, options: StreamOptions, client: Arc<dyn StreamHttpClient>) -> AssistantMessageEventStream {
    let url = build_url(&model, &options); let headers = build_headers(&model, &options); let body = build_request_body(&model, &context, &options);
