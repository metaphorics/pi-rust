...
pub fn build_headers(model: &Model, options: &StreamOptions) -> Vec<(String, String)> {
    ...
    if let Some(key) = &options.api_key { headers.push(("x-goog-api-key".into(), key.clone())); }
    headers
}

pub fn build_url(model: &Model) -> String { format!("{}/models/{}:streamGenerateContent?alt=sse", model.base_url.trim_end_matches('/'), model.id) }
...
pub fn stream_with_client(model: Model, context: Context, options: StreamOptions, client: Arc<dyn StreamHttpClient>) -> AssistantMessageEventStream {
    let url = build_url(&model); let headers = build_headers(&model, &options); let body = build_request_body(&model, &context, &options);
