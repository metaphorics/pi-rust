...
pub fn build_request_body(model: &Model, context: &Context, options: &StreamOptions) -> Value {
    let tools: Vec<Value> = context.tools.iter().map(|tool| json!({
        "name": tool.name,
    ...
    if let Some(system) = &context.system_prompt { body["system"] = Value::String(system.clone()); }
...
where I: IntoIterator<Item=B>, B: AsRef<[u8]> {
    ...
    for data in parse_sse_chunks(chunks) {
        if data == "[DONE]" { break; }
        let event: Value = serde_json::from_str(&data).map_err(|error| format!("invalid Anthropic SSE JSON: {error}"))?;
        match event["type"].as_str() {
            Some("message_start") => {
                ...
                builder.set_usage(usage["input_tokens"].as_u64(), usage["output_tokens"].as_u64(), usage["cache_read_input_tokens"].as_u64(), usage["cache_creation_input_tokens"].as_u64(), None);
            }
            Some("content_block_start") => {
                ...
                match block["type"].as_str() {
                    Some("tool_use") => {
                        ...
                        builder.tool_call_start(&key, block["id"].as_str().unwrap_or(""), block["name"].as_str().unwrap_or(""));
                        if let Some(input) = block["input"].as_object() {
                            let json = serde_json::to_string(input).unwrap_or_default();
                            if json != "{}" { builder.tool_call_delta(&key, &json); }
                        }
                    }
                    Some(kind) => { blocks.insert(index, kind.to_owned()); }
                    None => {}
                }
            }
            Some("content_block_delta") => {
                ...
                match delta["type"].as_str() {
                    ...
                    Some("thinking_delta") => builder.thinking_delta(delta["thinking"].as_str().unwrap_or("")),
                    Some("signature_delta") => builder.set_thinking_signature(delta["signature"].as_str().unwrap_or("").to_owned()),
                    Some("input_json_delta") => {
                        let key = blocks.get(&index).cloned().unwrap_or_else(|| index.to_string());
            ...
            Some("message_delta") => {
                reason = common::stop_reason(event.pointer("/delta/stop_reason").and_then(Value::as_str));
                builder.set_usage(None, event.pointer("/usage/output_tokens").and_then(Value::as_u64), None, None, None);
            }
            Some("error") => return Err(event.pointer("/error/message").and_then(Value::as_str).unwrap_or("Anthropic stream error").to_owned()),
            _ => {}
        }
    }
