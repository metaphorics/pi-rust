    ...
    B: AsRef<[u8]>,
{
    ...
    for data in parse_sse_chunks(chunks) {
        ...
        match event["type"].as_str() {
            ...
            Some("thinking_end") => {
                if let Some(signature) = event["contentSignature"].as_str() {
                    builder.set_thinking_signature(signature.to_owned());
                }
            }
            Some("toolcall_start") => {
                let key = event["contentIndex"].as_u64().unwrap_or(0).to_string();
                builder.tool_call_start(
                    &key,
                    event["id"].as_str().unwrap_or(&key),
            ...
            Some("done") => {
                reason = common::stop_reason(event["reason"].as_str());
                ...
                builder.set_usage(
                    ...
                    usage["reasoning"].as_u64(),
                );
                builder.set_response_id(event["responseId"].as_str());
            }
            Some("error") => {
                return Err(event["errorMessage"]
                    .as_str()
                    .unwrap_or("pi-messages stream error")
                    .to_owned());
            }
            _ => {}
        }
