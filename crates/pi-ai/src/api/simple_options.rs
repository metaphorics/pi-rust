...
pub fn build_base_options(
    ...
    result.max_tokens = Some(clamp_max_tokens_to_context(
        model,
        context,
        result.max_tokens.unwrap_or(model.max_tokens),
    ));
    if api_key.is_some() {
...
pub fn adjust_max_tokens_for_thinking(
    ...
    let budget = match level {
        ...
        ThinkingLevel::High | ThinkingLevel::Xhigh | ThinkingLevel::Max => {
            custom.and_then(|v| v.high).or(defaults.high)
        }
    }
    .unwrap_or(1024);
    let max_tokens = base_max_tokens.map_or(model_max_tokens, |base| {
        base.saturating_add(budget).min(model_max_tokens)
    });
