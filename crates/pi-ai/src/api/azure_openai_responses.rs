...
pub fn build_headers(model: &Model, options: &StreamOptions) -> Vec<(String, String)> {
    ...
    if let Some(key) = &options.api_key { headers.push(("api-key".into(), key.clone())); }
    headers
}

pub fn build_url(model: &Model, options: &StreamOptions) -> String {
    let version = options.metadata.as_ref().and_then(|m| m.get("apiVersion")).and_then(Value::as_str).unwrap_or(DEFAULT_API_VERSION);
    let base = model.base_url.trim_end_matches('/');
    let path = if base.ends_with("/responses") { base.to_owned() } else { format!("{base}/openai/responses") };
