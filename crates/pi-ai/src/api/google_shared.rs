    ...
    B: AsRef<[u8]>,
{
    ...
    for data in data_events {
        ...
        let response: Value = serde_json::from_str(&data)
            .map_err(|error| format!("invalid Google stream JSON: {error}"))?;
        ...
        let Some(candidate) = response["candidates"].as_array().and_then(|v| v.first()) else {
            continue;
        };
            ...
            .and_then(Value::as_array)
        {
            for (index, part) in parts.iter().enumerate() {
                ...
                if let Some(call) = part.get("functionCall") {
                    ...
                    if let Some(signature) = part["thoughtSignature"].as_str() {
                        builder.set_tool_signature(&key, signature.to_owned());
                    }
                }
            }
        }
        if candidate["finishReason"].is_string() {
            reason = common::stop_reason(candidate["finishReason"].as_str());
        }
    }
