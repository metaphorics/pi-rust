...
use crate::{
    ...
    types::{AssistantMessageEvent, Model, StopReason},
};

use super::common::{self, ApiResult, EventBuilder};

pub fn parse_responses_stream<I, B>(
    chunks: I,
    model: &Model,
    ...
    for data in parse_sse_chunks(chunks) {
        ...
        match event["type"].as_str() {
            ...
            Some("response.output_text.delta") => {
                builder.text_delta(event["delta"].as_str().unwrap_or(""))
            }
            Some("response.reasoning_summary_text.delta" | "response.reasoning_text.delta") => {
                builder.thinking_delta(event["delta"].as_str().unwrap_or(""))
            }
            Some("response.output_item.added") => {
                ...
                if item["type"] == "function_call" {
                    ...
                    if let Some(args) = item["arguments"].as_str().filter(|v| !v.is_empty()) {
            ...
            Some("response.reasoning_summary_part.added") => {
                if let Some(signature) = event
                    .pointer("/part/encrypted_content")
