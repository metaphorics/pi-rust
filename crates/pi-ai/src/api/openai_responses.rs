...
pub fn build_request_body(model: &Model, context: &Context, options: &StreamOptions) -> Value {
    ...
    if model.reasoning { body["reasoning"] = json!({"effort":"medium","summary":"auto"}); }
    body
}
...
pub fn stream(model: Model, context: Context, options: StreamOptions) -> AssistantMessageEventStream {
    match ReqwestStreamHttpClient::new() {
        ...
        Err(error) => {
            ...
            let mut message = common::empty_message(&model); message.stop_reason = StopReason::Error; message.error_message = Some(error.to_string());
            stream.push(AssistantMessageEvent::Error { reason: StopReason::Error, error: message }); stream
        }
    }
}
