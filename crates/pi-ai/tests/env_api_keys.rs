...
fn key_precedence_and_provider_map_match_pi() {
    ...
    assert_eq!(
        find_env_keys("anthropic", Some(&values)).unwrap(),
        vec!["ANTHROPIC_OAUTH_TOKEN", "ANTHROPIC_API_KEY"]
    );
    assert_eq!(
        get_env_api_key("anthropic", Some(&values)).as_deref(),
        Some("oauth")
    );
}
...
fn bedrock_reports_ambient_authentication() {
    ...
    assert_eq!(
        get_env_api_key("amazon-bedrock", Some(&values)).as_deref(),
        Some("<authenticated>")
    );
}
...
fn vertex_requires_adc_project_and_location() {
    ...
    assert_eq!(
        get_env_api_key("google-vertex", Some(&values)).as_deref(),
        Some("<authenticated>")
    );
