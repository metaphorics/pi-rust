...
fn temp_auth_path() -> PathBuf {
    std::env::temp_dir().join(format!("pi-ai-auth-{}-{}.json", std::process::id(), jiff::Timestamp::now().as_nanosecond()))
}

#[tokio::test]
async fn file_store_roundtrips_auth_json_and_enforces_private_mode() {
    let path = temp_auth_path();
    tokio::fs::write(&path, include_str!("fixtures/auth/sample_auth.json")).await.unwrap();
    ...
    let expected: Value = serde_json::from_str(include_str!("fixtures/auth/sample_auth.json")).unwrap();
...
fn auth_fixture_preserves_extension_fields() {
    ...
    let Credential::OAuth(oauth) = &auth["anthropic"] else { panic!("expected oauth") };
